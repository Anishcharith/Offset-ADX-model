...
def ADX(comp,day):
    ...
    global lowp
    global highp
    closep=list(np.flipud(data['Close'].values)[:-1*day])
    openp=list(np.flipud(data['Open'].values)[:-1*day])
    highp=list(np.flipud(data['High'].values)[:-1*day])
    lowp=list(np.flipud(data['Low'].values)[:-1*day])
